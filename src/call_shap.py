...
def cal_shap(model, X_test,SHAP_PLOT_TYPE,actions):
    path=save_path()
    ...
    for x in range(len(actions)): 
        ...
        shap.summary_plot(shap_values[:, :, x],X_test,plot_type=SHAP_PLOT_TYPE,show=False)
        plt.savefig(f"{path}/Action's{actions[x]}_shap.png")
        plt.close()
